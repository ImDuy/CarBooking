...
export default function useClerkSignUp() {
  ...
  const clerkSignUp = useCallback(
    async (signUpForm: AuthFormInfo) => {
      if (!isLoaded) return;
    ...
    [isLoaded, signUp, dispatch]
  );

  const clerkVerification = useCallback(
    async (code: string) => {
      if (!isLoaded) return;
    ...
    [isLoaded, signUp, dispatch, userInfo, setActive]
  );
